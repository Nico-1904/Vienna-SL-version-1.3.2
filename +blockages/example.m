...
wu = blockages.WallBlockage(cornerList, wallLossdB);
...
% x Direction
manhattanCity.xSize = 100;
% y Direction
manhattanCity.ySize = 100;
% spacing between buildings for the street in meter
manhattanCity.streetWidth = 20;
% define the boundaries of the random heights of the buildings
manhattanCity.minBuildingHeight = 10;
manhattanCity.maxBuildingHeight = 100;

% define borders
placementRegion = parameters.regionOfInterest.Region();
placementRegion.xSpan = 500;
placementRegion.ySpan = 500;
placementRegion.zSpan = 150;
placementRegion.origin2D = [250; 250];

% invoke the constructor to create the object
city = blockages.ManhattanCity(manhattanCity, placementRegion);

% plots the created city
if showPlots
    figure(3);
    title('Manhattan city with square buildings and 20 m street width');
    grid on;
    city.plot(grey + 0.1); % color buildings brighter than streets
end
...
cityParameter.longitude   = [16.368, 16.372];
% latitude coordinate
cityParameter.latitude    = [48.198, 48.199];
...
city = blockages.OpenStreetMapCity(cityParameter, placementRegion);
