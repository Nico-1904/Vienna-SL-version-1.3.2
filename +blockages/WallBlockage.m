classdef WallBlockage < blockages.Blockage
    ...
    methods (Static)
        function newWallBlockages = generatePredefinedPositions(wallParameter, ~)
            ...
            newWallBlockages(nWalls) = blockages.WallBlockage();
            positionsMat  = repmat(wallParameter.positions,1,1,size(wallParameter.cornerList,2));

            % set wall blockages
            for iWall = 1:nWalls
                % set creat wall obj
                newWallBlockage = blockages.WallBlockage(squeeze(positionsMat(:,iWall,:)) + wallParameter.cornerList, wallParameter.loss);
